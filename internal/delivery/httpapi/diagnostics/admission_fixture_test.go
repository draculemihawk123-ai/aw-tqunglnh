...
// admission_test.go's writeAdmissionExecutable/admissionPinnedBuild) and
// internal/delivery/httpapi/recovery's own identical duplicate of the same
// helpers — the same "Go test helpers are not exported across packages,
// duplicated locally" discipline that package's own admission_fixture_test.go
// doc comment already documents. Every helper drives a REAL *sqlite.Store
// through the REAL application commands — never a hand-seeded row.
//
// Unlike recovery's own version, blockedAdmissionFixture here uses the REAL
// production process.NewIsolationChecker() (internal/adapters/process),
// never a fake/toggle: that checker's own documented, static, I/O-free
// contract (ADR-013/ADR-023 — "OPERATOR_TRUSTED_LOCAL is the only tier this
// environment can run today... always rejects ENFORCED_ISOLATED honestly")
// already rejects the ENFORCED_ISOLATED tier permissionPolicyDocument below
// pins, which is all this package's own tests need: a genuine, real,
// non-faked ISOLATION_ENFORCEMENT_UNAVAILABLE admission blocker. recovery's
// own toggleIsolationChecker exists only because ITS tests need a later
// RETRY to flip from failing to passing — this package never retries
// anything (diagnostics is read-only), so no toggle is needed here.
...
func validAgentProfileDocument() agentprofile.AgentProfileDocument {
	return agentprofile.AgentProfileDocument{
		ProviderKey: "claude", Model: "fake-model", ToolRefs: []string{"read_file"},
...
// blockedAdmissionFixtureResult is blockedAdmissionFixture's own output.
...
// every "genuinely admission-BLOCKED NodeRun" HTTP diagnostics test: a
// real, sqlite-backed AGENT NodeRun admission-BLOCKED for
// ISOLATION_ENFORCEMENT_UNAVAILABLE, via the real scheduling pipeline
// (StartWorkflowRun -> AdvanceRun -> ScheduleExecutableNodeRun -> a REAL
// claimed EXECUTE_NODE job -> a real runtime.ExecuteNodeHandler.Handle call
// against the REAL production process.NewIsolationChecker()).
func blockedAdmissionFixture(t *testing.T, dbName string) blockedAdmissionFixtureResult {
	t.Helper()
	store, uow := openDiagnosticsTestStore(t, dbName)
	...
	executablePath := writeAdmissionExecutable(t, "diagnostics-fixture-binary-v1")
	...
	return blockedAdmissionFixtureResult{
		...
		BlockerID: scheduled.AttemptID + "-admission-blocker", Registry: registry,
	}
}

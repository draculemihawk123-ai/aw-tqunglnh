...
// global. Mirrors internal/delivery/httpapi.Config's own "everything a
// handler needs travels through one explicit struct" shape, scoped to this
// package's own narrower dependency set (a UnitOfWork, an ID source and a
// Clock — no HTTP server/listener concern belongs here, that stays
// composition-root/V6-12 territory).
type Dependencies struct {
	...
	// (CreateRootWorkItem/CreateChildWorkItem/RequestScopeExpansion all take
	// an idsource.Source themselves). A composition root supplies

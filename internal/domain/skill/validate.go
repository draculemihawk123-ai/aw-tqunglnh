...
// at the first — an author fixing a Skill from scratch needs the whole
// picture in one pass.
func ValidateDocument(doc SkillDocument) authoring.Diagnostics {
	...
	if len(doc.Resources) == 0 {
		diags = append(diags, authoring.Diagnostic{
			...
			Why:  "a Skill that publishes nothing can never be selected for anything",
...
func validateResource(path string, resource Resource) authoring.Diagnostics {
	...
	if strings.TrimSpace(resource.Instruction) == "" {
		diags = append(diags, authoring.Diagnostic{
			Path: path + ".instruction",
			What: "instruction is empty",
			Why:  "a resource with no instruction content has nothing for an attempt to load or follow",
			Fix:  "set instruction to the text this resource should contribute",
		})
	}
